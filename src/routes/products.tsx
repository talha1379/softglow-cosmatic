  ...
  head: () => ({
    meta: [
      { title: "Products — SoftGlow 3D Showcase" },
      {
        name: "description",
        content:
          "Explore our luxury skincare collection in immersive 3D. Serums, creams, mists and more.",
      },
    ],
  }),
});
...
function Products() {
  ...
  const [pendingActive, setPendingActive] = useState<number | null>(null);
  const [showLoader, setShowLoader] = useState(false);

  useEffect(() => {
    if (!showLoader || pendingActive === null) return;
  ...
  }, [pendingActive, showLoader]);

  return (
    <Layout>
      <MiniGameLoader open={showLoader} onComplete={() => setShowLoader(false)} />
      <AnimatedSection className="bg-hero-gradient py-16">
        <div className="mx-auto max-w-7xl px-6 text-center">
          <p className="text-sm font-semibold text-gold uppercase tracking-widest mb-3">
            3D Collection
          </p>
          <h1 className="font-display text-5xl md:text-6xl font-bold mb-4">
            Touch beauty, virtually.
          </h1>
          <p className="text-muted-foreground max-w-xl mx-auto">
            Drag, rotate and explore each product in stunning detail before it touches your skin.
          </p>
        </div>
      </AnimatedSection>

      <AnimatedSection className="mx-auto max-w-7xl px-6 py-12">
          ...
          <div className="scroll-smooth overflow-x-auto pb-4 flex gap-6 snap-x snap-mandatory">
            {products.slice(0, 6).map((p, i) => (
              <div
                key={p.id}
                onClick={() => {
                  setPendingActive(p.id);
                  setShowLoader(true);
                }}
                className="group cursor-pointer min-w-[320px] snap-start relative bg-card rounded-3xl overflow-hidden border border-border hover:shadow-luxe transition-all hover:-translate-y-1"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="text-xs px-4 py-2 rounded-full bg-foreground text-background hover:opacity-90">
                      Quick Add
                    </button>
                  </div>
                </div>
      ...
      {active !== null && (
        <div
          className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur-sm grid place-items-end"
          onClick={() => setActive(null)}
        >
          <div
            className="bg-card w-full max-w-md h-full p-8 overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <button onClick={() => setActive(null)} className="text-sm text-muted-foreground mb-6">
              ← Close
            </button>
            <img
              src={productImgs[active]}
              alt=""
            ...
            <p className="text-2xl text-gold-gradient font-semibold mb-4">
              {products[active].price}
            </p>
            <p className="text-muted-foreground mb-6">
              A luxurious formula crafted with botanical extracts and clinically-proven actives for
            ...
            <button className="w-full py-4 rounded-full bg-foreground text-background font-medium">
              Add to Routine
            </button>
          </div>
        </div>
